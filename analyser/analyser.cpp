...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while (true) {
            ...
            if(next.value().GetType() == TokenType::EQUAL_SIGN){
                ...
                _instructions.emplace_back(Operation::STO, _vars[tmp.value().GetValueString()]);
                // ';'
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if(next.value().GetType() == UNSIGNED_INTEGER){
		    if(next.value().GetValueString().length() <= 10) {
                st << next.value().GetValueString();
                st >> flowtest;
                if (flowtest <= 2147483647) {
                    st << next.value().GetValueString();
                    st >> res;
                    out = res;
                } else {
		...
		switch (next.value().GetType()) {
		    ...
		    case IDENTIFIER:{
                ...
                if(_vars.find(next.value().GetValueString()) == _vars.end())
                    value = _consts[next.value().GetValueString()];
                else
                    value = _vars[next.value().GetValueString()];

                _instructions.emplace_back(Operation::LOD, value);
		    ...
		    case UNSIGNED_INTEGER:{
                ...
                if(next.value().GetValueString().length() <=10 ){
                    ...
                    if(prefix == 1 && num >2147483647)
                        return std::make_optional<CompilationError>(_current_pos,ErrorCode::ErrIntegerOverflow);
                    else if(prefix == -1 && num >2147483648)
                        return std::make_optional<CompilationError>(_current_pos,ErrorCode::ErrIntegerOverflow);
                    else{
                        n = num;
                        _instructions.emplace_back(Operation::LIT, prefix);
                        _instructions.emplace_back(Operation::LIT, n);
                        _instructions.emplace_back(Operation::MUL, 0);
                    }
                }
                else
